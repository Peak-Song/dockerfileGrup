...
redis_cluster_override_conf() {
    ...
    if ! (is_boolean_yes "$REDIS_CLUSTER_DYNAMIC_IPS" || is_boolean_yes "$REDIS_CLUSTER_CREATOR"); then
        ...
        ! [[ -z "REDIS_CLUSTER_ANNOUNCE_PORT" ]] && redis_conf_set cluster-announce-port "$REDIS_CLUSTER_ANNOUNCE_PORT"
        ! [[ -z "REDIS_CLUSTER_BUS_ANNOUNCE_PORT" ]] && redis_conf_set cluster-announce-bus-port "$REDIS_CLUSTER_BUS_ANNOUNCE_PORT"
    fi
    if is_boolean_yes "$REDIS_CLUSTER_DYNAMIC_IPS"; then
